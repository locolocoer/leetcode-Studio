...
function cppHarness(problem, sourceCode, className) {
  ...
  if (problem.judgeType === "class") {
    const ctorArgs = (problem.constructorParams ?? []).map((p, i) => {
      ...
      return cppLoadArg(shape, `JVal::parse(argvRaw[1], _off).arr[0].arr[${i}]`);
    }).join(", ");
  ...
  ${className} obj(${ctorArgs});
  vector<string> result; result.push_back("null");
  for (size_t i = 1; i < ops.size(); i++) {
    string op = ops[i];
    JVal _args_i = JVal::parse(argvRaw[1], _off).arr[i];
...
`;
  ...
  return 0;
    ...
    const args = problem.params.map((p, i) => {
      const expr = `JVal::parse(argvRaw[${i}], _off)`;
      return cppLoadArg(parseType(p.type), expr);
    }).join(", ");
    driver = `
int main(){
  string raw; { char c; while(cin.get(c)) raw += c; }
...
`;
  ...
  return {
    ...
    compile: { cmd: "gcc", args: ["-static", "main.c", "solution.c", "-o", "main"] },
    run: { cmd: process.platform === "win32" ? "./main.exe" : "./main", args: [] },
    className
  };
}
...
var minStackPy = `class MinStack:
    ...
    def getMin(self) -> int:
        return self.mins[-1]
`;
var minStackJava = `class MinStack {
...
if (!lang || !outdir) {
  console.error("usage: node harness-cli.mjs <lang> <outdir>");
  process.exit(2);
}
try {
...
function emit(problem, source, name) {

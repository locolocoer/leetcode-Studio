...
- **逐步调试面板精简**：去掉「执行轨迹（最近几步）」列表和「高亮行 = 即将执行…」的说明文字。
  停在哪一行直接看编辑器的蓝色高亮行即可，变量与程序输出上移，面板更干净。
  （异常仍会在状态区以红色文字显示，信息没有丢失。）

### 1.1.1

- **应用图标**：换成与界面一致的品牌图标（蓝紫渐变圆角方块 + `LC` 字标），此前用的是 Electron 默认图标。
  生效范围：桌面快捷方式、开始菜单、任务栏、窗口、`setup.exe` 与便携版 exe。
  多尺寸 ICO（16/24/32/48/64/128/256）：小尺寸用 32bpp BMP 条目、大尺寸用 PNG 条目，兼容老 API 不糊。
  重新生成：`powershell -File scripts/make-icon.ps1`。

### 1.1.0

**新增 🤖 AI 做题助手**（`AI 助手` 标签页，设置页可配置）

- **分级提示**：点「给我一个提示」每次只升一级——① 复述题目关键条件与目标 → ② 引导性提问 → ③ 思路方向 → ④ 步骤骨架 → ⑤ 只给卡点的 ≤3 行关键代码；第 5 级后不再升级。**绝不输出完整题解**。
- **纠错**：带上你的代码、失败用例（输入/期望/实际/报错）与调试暂停时的变量，先复述你的意图，再指出第一处问题与原因，并反问确认，不重写整个函数。
- **接入**：任意 OpenAI 兼容接口，内置 DeepSeek / OpenAI / Moonshot / 智谱 / 本地 Ollama 预设；Key 只存本机，请求由主进程直连。

## 发布产物

| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.2-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.2-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

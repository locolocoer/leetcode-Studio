...
**判题模板不再靠手写硬编码：遇到没见过的题型，让 AI 生成编译模板**

- 原来的判题驱动是「按题型穷举」写死的（函数型/class 型/链表/二叉树/相交链表…），遇到新形态就得改代码，这次 236 就是被这种元数据坑到。
- 现在的流程：
  1. 先用**确定性模板**跑（离线、秒级、结果稳定，绝大多数题都走这条）；
  2. 只有出现「模板疑似不适配」的信号（编译失败，或所有用例都没过且不同输入的实际输出完全一样）才找 AI；
  3. AI 生成的驱动**必须通过本题全部用例**才会被采用，并按「题目 + 语言 + 签名」缓存，之后秒开、不再调用模型；
  4. 生成不通过就**如实回退**到原结果并把失败原因写清楚，绝不假装成功。
- 安全与防作弊约束：AI 只能写驱动（不能碰你的解答文件）；必须真的调用你的解答方法；禁止进程/网络/文件操作；样例期望值不允许硬编码成字面量；全部用例通过才接受。
- 界面：设置 →「AI 助手」新增开关**「遇到没见过的题型时，让 AI 生成编译模板」**（默认开启，需配置 API Key）与「清空 AI 模板缓存」；运行时顶部会显示「AI 正在生成判题模板…」，结果区会出现 `🤖 使用了 AI 生成的判题模板` 徽标。
- 说明：**需要先在设置里填 API Key**（DeepSeek/OpenAI/Moonshot/GLM/本地 Ollama 都行）。不填也能正常用，只是仍走确定性模板。
- 新增回归脚本 `scripts/ai-harness-test.ts`：用打桩模型验证「触发 → 生成 → 验证 → 采用 → 缓存命中 → 拒绝并回退」整条管线（无需 API Key）。
...
| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.8-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.8-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

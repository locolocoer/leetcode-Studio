...
**「拉取题单」不再只有两个：加入官方学习计划 + 你自己的题单**

以前那个弹窗里写死了两个收藏夹题单，其它全靠自己粘链接。现在分成四块：

| 分组 | 内容 |
| --- | --- |
| **我的题单** | 登录后自动列出**你自己创建的题单**（收藏夹），名字就是你在力扣上起的名字 |
| **学习计划** | 官方分阶段题单：面试经典 150 题（150）· LeetCode 75（75）· 编程基础 0 到 1（33）· 动态规划（基础版，46）· 二分查找 · 系统掌握（32）· 图论 · 从入门到精通（36）· 30 天 JavaScript 挑战（30） |
| **官方题单** | LeetCode 热题 HOT 100（99）· LeetCode 200 题（200） |
| **其它 / 自定义** | 粘贴题单链接或 ID；`/studyplan/xxx` 与 `/problem-list/xxx` 都能识别 |

- 学习计划走的是官方 `studyPlanV2Detail` 接口，**名字与题量都是接口里的真实数据**（不是猜的），按分组顺序合并成一份题单，拉下来就是左侧一个分类，能直接按顺序刷。
- 拉取后同样支持刷题记录、按题单清除记录、进度统计（✓ 已通过 / 总数）。
- 新增回归脚本 `scripts/study-plan-test.ts`：校验内置学习计划清单、逐个真实拉取并检查中文标题与题量。
...
| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.16-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.16-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

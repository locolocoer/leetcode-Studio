...
**题解：显示作者头像 + 昵称，代码块彩色高亮**

- 题解详情顶部现在有**作者信息行**：头像、昵称（力扣的 realName，如「力扣官方题解」）、`@用户名`、点赞数与发布时间，「主页 ↗」可直接打开 TA 的力扣主页。
- 左侧题解列表每一条也带**小头像与昵称**，一眼看出是谁写的。
- 头像取不到时（离线 / CDN 挂了）自动降级成**昵称首字彩色圆**，不会出现破图。
- 代码块接入 **highlight.js**：C / C++ / Java / Python / JS / TS / Go / Rust / C# / Kotlin / Swift / Ruby / PHP / SQL / Shell 高亮；题解里没写语言的代码块会在这几种里自动推断，仍拿不准就按纯文本显示。「复制」按钮不受影响。
- 顺手把高亮主题调成与界面一致的深色配色（关键字、字符串、注释、类型各自配色）。
...
| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.10-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.10-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

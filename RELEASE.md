...
- 根因一（真凶）：gdb 的命令是「先发送、后注册等待响应」，而 gdb 的响应可能在注册之前就到达并被丢弃，于是每个丢响应的命令都要白等一次超时（9 秒），断点/继续/步过叠加起来就像卡死。现在统一改成**先注册等待再发送**（新增 `sendWait`），并给 jdb 侧同样修正了「基准行号取晚了会漏读输出」的问题。
- 根因二：调试器发出的**错误事件没有被界面消费**，状态一直停在「运行中」，看起来就是没反应。现在 `error` 事件会直接变成红色错误提示（例如步进超时、gdb 无响应）。
- 另外把单步/步过/继续的循环改成**总时间预算**（步进/步过 25 秒、继续 60 秒，单次响应 8~15 秒），超时会给出可操作的提示（「这一行进入了很深的库代码，可改用继续或把断点放到外层」），不会再无限等待。
- 回归脚本 `scripts/step-over-test.ts`：threeSum（含 `sort` + 嵌套循环 + `vector`）连续 12 次步过，每次约 120ms，行号与变量都正确。
- 新增 `LC_GDB_DEBUG=1` 时打印完整 gdb MI 交互流，便于日后排查。
...
| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.4-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.4-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

...
**修复「最近公共祖先」这类题编译不过**（`invalid conversion from 'int' to 'TreeNode*'`）

- 原因：LeetCode 的元数据把 `p`/`q` 写成 `integer`、测试用例给的是节点值（`5`/`1`），而真实签名要的是 `TreeNode*`；本地判题按元数据生成了 `int`，于是编译报错。
- 修复：现在会**解析你写的真实函数签名**判断哪个参数是「按值引用的节点」，然后：
  1. 先用前面的树/链表参数建出结构；
  2. 在其中按值找到对应节点，把**真实节点指针**传进去；
  3. 返回值是节点引用时（期望值是 `3` 这种标量）**按节点值比较**，而不是打印整棵树。
- 三种语言都支持：C++（`_lcFindTree`）、Java（`Json.findTreeNode`）、Python（`_lc_find_tree`），运行与调试两条路径一致。
- 覆盖题目：236 / 235 最近公共祖先、1644 / 1676 等 LCA 变体、1379 之类「传节点」的题型；返回整棵树的题（如 226 翻转二叉树）不受影响。
- 顺带修掉：**Java 代码里写中文注释会报 GBK 编译错误** —— 现在 `javac` 显式使用 `-encoding UTF-8`（运行与调试路径都改了）。
- 新增回归脚本 `scripts/node-ref-test.ts`（236 三语言 + 226 对照）与 `scripts/lca-236-test.ts`（现场复现：运行 + 调试，`p`/`q` 现在是真正的树节点，可展开看 `val/left/right`）。
...
| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.7-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.7-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

...
**修复调试时「编译失败：ld.exe: cannot open output file main.exe: Permission denied」**

- 根因：Windows 上结束调试时杀掉的是 gdb / jdb，**被调试的程序（main.exe / java.exe）不会被带走**，残留进程锁住会话目录里的产物，下一次链接/编译就写不进去。
- 修复三处：
  1. 结束调试改为结束**整棵进程树**（`taskkill /T /F`），从根上不再留下残留进程（Java 的 JVM 也一并处理）。
  2. 开始新会话前先彻底结束上一次会话（停掉 gdb/jdb 与被调试进程），再清理会话目录。
  3. 编译/链接仍被占用时**自动恢复**：先按目录结束残留进程再重试；若仍被占用则换一个输出文件名继续（C/C++），Java 编译同样会先清理再重试。
- 运行（非调试）路径也加了同样的占用重试逻辑。
- 新增回归脚本 `scripts/debug-lock-test.ts`：会真的造一个占用 `main.exe` 的进程，验证「首次链接失败 → 自动清理 → 重试成功」以及 dispose 后无残留进程。

### 1.1.2

- 逐步调试面板去掉「执行轨迹（最近几步）」列表与「高亮行 = 即将执行…」说明，面板更干净。

### 1.1.1

- 应用图标换成品牌图标（蓝紫渐变 + `LC`），覆盖桌面快捷方式 / 开始菜单 / 任务栏 / 窗口 / 安装包。

### 1.1.0

- 新增 **🤖 AI 做题助手**：分级提示（绝不直接给答案）与纠错，可带代码/失败用例/调试变量。

## 发布产物

| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.3-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.3-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

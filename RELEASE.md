...
- 修复「最大化后还原，右侧编辑器宽度不回收」：还原后整页被撑到比窗口宽（1440 的窗口里页面宽 1949），右栏与编辑器还停在最大化时的宽度，顶栏的「提交」等按钮被挤出可视区。原因是右栏作为栅格项用了默认的 `min-width: auto`，下限被编辑器当前宽度顶住，窗口变小也压不下去。现在右栏能正常收缩，编辑器跟着回收并重新排版。
- 新增界面回归 `scripts/ui-layout-check.mjs`：连做两轮「最大化 → 还原」，校验右栏/编辑器宽度回到基准、整页无横向溢出、顶栏按钮都在可视区内，再把视口压到最小窗口宽度复核一次。

### 1.1.21

- 修复「镜像里没有更新版本时再去 GitHub Releases 确认」这条兜底路径：上一次检查还没结束就发起新检查时，会被直接忽略，导致兜底实际没生效；现在改为等本轮检查结束后再查，并在日志里留下记录。
- 顺带把共享库条目的文件名规整了：题号 + 语言 + 签名哈希之间统一用 `-` 连接（此前哈希为负数时会多出一个横线，例如 `1-cpp--700170664` → `1-cpp-700170664`）。
...
以前内置模板覆盖不到的题型，每个用户都要各自让 AI 生成一遍（还得自己配 API Key）。现在模板可以**发布**到
仓库里的共享库（`harness/shared/`），其他人遇到同一题型会自动拉取复用。

**使用顺序**（运行与调试都一样）：

1. 你自己编辑的模板
2. 本地已缓存并验证过的（AI 生成的 / 从共享库拉取的）
3. 内置确定性模板
4. **共享模板库** ← 内置模板编译不过时先来这里找：从镜像（OSS）拉取，失败回退 GitHub raw；拉下来会**本地编译 + 跑一遍全部用例**，通过才采用并缓存（缓存后不再联网）
5. AI 生成（需要 Key；现在最多 3 轮，会把编译错误和上一版驱动一起回给模型修正）

**发布模板**：运行结果区「🧩 查看/编辑模板」→ 改好并验证 → 点「**发布到共享库**」。

- 设置里填一个 GitHub Token（只需本仓库 contents 写权限）就能直接提交：会写入 `harness/shared/<题目>-<语言>-<签名>.json` 并更新 `harness/shared/index.json`。
- 没填 Token 也行：会**导出到本地目录**，你可以自己把那两个文件提交到仓库（PR 也欢迎）。
- 提交后 GitHub Action 会把 `harness/shared/` 同步到对象存储；客户端优先走镜像，加速国内拉取。
- 设置里可以关掉「共享模板库」，也可以一键「重置判题模板」。

新增回归 `scripts/harness-registry-test.ts`：拉取 → 本地验证 → 采用 → 命中缓存不联网，以及发布的两种路径（导出 / GitHub API 提交模板与索引）。

### 1.1.18

- 修复 `character[][]` 字符矩阵题型（最大正方形等）；调试路径也具备 AI 自动修正。
...
| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.22-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.22-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

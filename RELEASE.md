...
**判题模板可以自己看、自己改（最后一道保障）**

- 运行结果区新增 **🧩 查看/编辑模板**：打开的编辑器里就是这道题当前生效的判题驱动（只展示可编辑的驱动部分，固定脚手架可折叠查看）。
- 改完点 **「验证并保存」**：会用本题全部用例当场跑一遍，结果显示在弹窗里；保存后这道题就一直用你这份模板，并且**不会再让 AI 介入**。
- 优先级：**你手写的模板 > AI 生成并验证过的模板 > 内置确定性模板**；编辑器里能看到当前来源，也能一键「恢复内置模板」。
- 编译失败时，错误下方直接给「🧩 打开判题模板编辑器」，不用先去别处找入口。
- 本地模板缓存文件：`%APPDATA%\leetcode-studio\.leetcode-studio\ai-harness.json`（按题目 + 语言 + 函数签名区分；设置里可整体清空）。
- 新增回归脚本 `scripts/harness-edit-test.ts`：内置模板失败 → 查看 → 手写驱动 → 保存 → 运行生效（且不调用 AI）→ 恢复默认。

### 1.1.8

- 判题模板可由 AI 生成：确定性模板不适配时自动兜底，**通过本题全部用例才采用**，并按题目缓存；不通过如实回退。
...
| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.9-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.9-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

...
界面上不该出现讲实现细节的话，这次统一收拾了一遍：

| 位置 | 原文 | 现在 |
| --- | --- | --- |
| 拉取题单 · 学习计划 | 「力扣官方分阶段题单，带真实名字与题量」 | 去掉（标题「学习计划」已说明一切） |
| 拉取题单 · 经典题单 | 「收藏夹形式的经典题单」 | 去掉，分组名改为「经典题单」 |
| 拉取题单 · 其它 | 「支持粘贴链接：problem-list/xxx 或 studyplan/xxx」 | 「粘贴题单链接，或直接填题单 ID」 |
| 拉取题目 | 「首次加载中文题目列表需要几秒，之后会用本地缓存」 | 去掉（加载动画已经说明在加载） |
| 运行结果 · 判题模板徽标 | 「AI 生成并通过本题用例验证（已缓存）」 | 只显示「🤖 AI 生成的判题模板」/「🧩 你自定义的判题模板」 |
| 设置 · AI 助手 | 「遇到没见过的题型时，让 AI 生成编译模板（必须通过本题全部用例才会采用，并按题目缓存）」 | 「遇到没见过的题型时，让 AI 生成判题模板」 |
| 设置 · AI 助手 | 按钮「清空 AI 模板缓存」 | 「重置判题模板」 |
| 侧栏未拉取的题 | 「未缓存 · 点击加载」 | 「点击加载题面」 |
| 调试空面板 | 「命中后会高亮即将执行的那一行，右侧实时显示变量值」「支持 Python（原生跟踪）· C / C++（gdb）· Java（jdb）」 | 「在编辑器里按 F9（或点行号）加断点，再点左侧「🐞 逐步调试」」「支持 Python · C / C++ · Java，工具链已内置」 |

原则：界面上只保留**用户需要知道的操作与结果**；讲「我们怎么实现的」放到文档里（README / RELEASE.md）。
...
| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.17-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.17-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

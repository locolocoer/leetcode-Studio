...
**修复「最大正方形」这类字符矩阵题编译不过，并让调试也具备 AI 自动修正**

问题：力扣元数据把字符矩阵写成 `character[][]`，而类型映射表里没有 `character`，落到了默认的 `int`，
于是生成了 `vector<vector<int>>` 去调用 `vector<vector<char>>`，编译报错。**不是 AI 生成错了——这条路径根本没走到 AI。**

1. **修好类型映射**：`character` / `char`（含 `character[]`、`character[][]`）现在正确映射为字符类型，三语言都能编译运行：
   - C++：`_vchar` / `_vvchar` 助手（同时补了 `string[][]` 的 `_vvstr` 与 `_ser(char)`）
   - Java：`Json.toChar` / `toCharArray` / `toCharMatrix`
   - Python 本来就按原始 JSON 传入，无需转换
2. **调试也有 AI 兜底了**：以前只有「运行」会自动修正模板，调试路径编译失败就直接报错。现在调试同样：优先用你手写的 / AI 已验证过的模板（含缓存），内置模板编译不过时自动让 AI 生成并**验证**，成功后继续调试。
3. **修正能力增强**：最多 3 轮；反馈时会把**编译错误/用例差异**连同**上一版驱动**一起给模型（让它改而不是推倒重来）。
4. 顺带修好字符数组的变量显示：`vector<char>` 以前显示成 `[5 项]`，现在展开能看到 `['1', '0', '1', '0', '0']`。

新增回归：`scripts/char-matrix-test.ts`（字符矩阵三语言运行 + C++ 调试）、`scripts/debug-ai-fix-test.ts`（调试路径 AI 自动修正：编译失败 → 生成 → 验证 → 继续调试 → 二次调试走缓存）。

### 1.1.17

- 清理界面上的实现说明文案（「带真实名字与题量」这类不该出现在 UI 的描述）。

### 1.1.16

- 拉取题单支持官方学习计划（面试经典 150 题、LeetCode 75 等 7 个）与「我的题单」。

### 1.1.15

- 代码补全能补全你自己写的变量、函数、类与成员（四语言）。
...
- 拉取题目列表跟随题面语言（中文用 `titleCn`），搜索支持中英文。
...
| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.18-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.18-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

...
- **应用图标**：换成与界面一致的品牌图标（蓝紫渐变圆角方块 + `LC` 字标），此前打包一直用的是 Electron 默认图标。
  - 生效范围：桌面快捷方式、开始菜单、任务栏、窗口、`setup.exe` 与便携版 exe 的图标。
  - 多尺寸 ICO（16/24/32/48/64/128/256）：小尺寸用 32bpp BMP 条目、大尺寸用 PNG 条目，兼容老 API 不糊。
  - 开发模式下窗口也会显示新图标；打包版额外设置了 AppUserModelId（`com.leetcodestudio.app`），任务栏分组/固定更正确。
  - 重新生成图标：`powershell -File scripts/make-icon.ps1`（源图 `build/icon.png`，输出 `build/icon.ico`）。
...
| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.1-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.1-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

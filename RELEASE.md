...
以前只补关键字、片段、题目参数和内置容器的成员，**你写的变量和函数一个都补不出来**。现在新增了文档符号提取
（`src/renderer/src/symbols.ts`），四语言都会解析当前文件：

| 能补什么 | 例子 |
| --- | --- |
| 局部变量 / 字段 | `int total = 0;` → 输入 `tot` 补出 `total | int` |
| 循环变量、函数参数 | `for (int i = …)`、`def twoSum(self, nums, target)` |
| 结构化绑定 / 元组解包 | C++ `for (auto& [k, v] : m)` → `k`、`v`；Python `left, right = 0, n-1` |
| 自己写的函数 / 方法 | `int helper(int a, int b)` → 输入 `hel` 补出 `helper | int helper(int a, int b)`（带参数占位） |
| 类 / 结构体 / 类型别名 | `struct Item`、`class Solution`、`typedef …`、`using … =` |
| 自定义类型的成员 | `Item best;` 之后输入 `best.` → 补出 `val | int · Item 的成员` |
| `#define` 宏、import 进来的名字 | `MAXN`、Java `import` 的类名 |
| Python 细节 | `with … as fh`、`except … as err`、`lambda a, b:`、海象 `:=`、类级字段 |

- 实现是轻量词法扫描（正则 + 括号/缩进配平），不依赖完整语法树：写完即补，不需要等语言服务。
- 排序上「你自己定义的符号」优先于关键字；成员补全会**先查你自己定义的类型**，再退回内置容器成员。
- 新增回归脚本 `scripts/symbols-test.ts`：四语言样例代码断言变量/函数/类型/成员提取结果。

### 1.1.14

- 修复侧栏「当前分类」被长分类名挤成多行的问题（改为两行结构 + 进度条）。
...
| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.15-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.15-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

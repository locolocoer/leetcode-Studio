...
**刷题记录：做过的题打 √，题单可以一键清零重新刷**

- **提交通过就记一笔**：在力扣上提交得到 Accepted 后，左侧列表该题右侧出现绿色 **✓**（悬停能看到是用哪种语言过的）。本地「运行所有用例」全过但还没提交的，用一个淡色小点提示，不算通过。
- **题单进度**：分类标签上会显示已通过数（如 `热题 100  100✓23`），当前分类那一行显示 `✓ 23 / 100`，一眼知道刷到哪了。
- **清除刷题记录**：
  - 切到某个题单/每日一题分类 → 点「**清除本单记录**」→ 该题单所有题的 √ 清零，重新开始刷（**代码和自定义用例都保留**）。
  - 在「全部题目」下可点「**清除记录**」清空全部。
- **刷题语言记住上次的选择**：以前每次打开都回到 Python，现在默认用你上次选的语言（顶栏那几个按钮只在需要切换时点）。选择会写进设置，重启也保持。
- 记录存在题库文件里（`problems.json` 的 `solvedAt/solvedLang/localPassAt`），并且做了保护：改代码、改用例、切换题面语言重拉题目都**不会**把记录抹掉。

### 1.1.12

- 拉取题目列表跟随题面语言（中文用 `titleCn`，4437/4443 题有中文名），搜索支持中英文；已存题面语言不一致时自动重拉。
...
| 文件 | 大小 | 说明 |
| --- | --- | --- |
| `LeetCode-Studio-1.1.13-setup.exe` | ≈172 MB | 安装版（推荐）：可自选目录、创建快捷方式、支持自动更新 |
| `LeetCode-Studio-1.1.13-portable.exe` | ≈172 MB | 免安装便携版：双击即用（首次启动需解压内置工具链到临时目录，约 1 分钟） |

...
2. **本地已缓存的模板**：本机之前用 AI 生成或者从共享库拉取过、且当时通过了全部用例的模板。
3. **内置确定性模板**（`src/main/harness.ts`）：按题目元数据 + 真实函数签名生成，离线、毫秒级，覆盖绝大多数题型。
4. **共享模板库**：`harness/shared/` 里的公共模板，先取 OSS 镜像、再回退 GitHub 原始文件，拉到本地验证（必须能编译并跑通本题用例）后才会使用。
5. **AI 生成并验证过的模板**：出现「模板疑似不适配」的信号（编译失败，或所有用例都没过且不同输入的实际输出完全一样）时，若已配置 AI Key 且开关打开，会调用 `src/main/aiHarness.ts` 生成驱动；**必须通过本题全部用例**才会被采用并按题目缓存。
...
- **同步**：条目进入 `harness/shared/` 后，CI 的 `sync-harness` 任务会把整个目录镜像到 OSS，之后所有用户都能从镜像拉到。

...
1. **确定性模板**（`src/main/harness.ts`）：按题目元数据 + 你写的函数签名生成驱动，离线、秒级，覆盖绝大多数题型。
2. 只有出现「模板疑似不适配」的信号（编译失败，或所有用例都没过且不同输入的实际输出完全一样）时，**若配置了 AI Key 且开关打开**，才调用 `src/main/aiHarness.ts` 让模型生成一份驱动。
3. 生成的驱动**必须通过本题全部用例**才会被采用，并按「题目 + 语言 + 签名」缓存到 `.leetcode-studio/ai-harness.json`；不通过就如实回退到原结果。
